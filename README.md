...
- **GPU-native multi-agent execution graph, in progress** — concurrent Claude Code sessions rendered as one live, interactive graph instead of a flat session list. Edges use RHI-backed rendering via Qt Quick Shapes (`Shape.CurveRenderer`) rather than stock shell widgets, because nothing off the shelf draws it the way it needs to move.
