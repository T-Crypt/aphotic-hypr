...
- **Maintenance tooling** — release tagging is live (GitHub Releases, starting at v2.0.0); an in-shell update check/apply flow and migration tooling are still ahead, tracked below.
- **In-shell update awareness** — a "check for updates" action on Settings → About that compares the installed `VERSION` against the latest GitHub release, and an "Update" action that runs the equivalent of `./install.sh --config-only` when one's available, instead of requiring a manual `git pull`. A native, schedulable "check for system package updates" control (Settings → System) is a related, separate piece — same idea, pacman/AUR instead of the dots repo.
