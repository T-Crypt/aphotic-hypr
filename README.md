...
| Layer | Adds |
|---|---|
| `gaming` | GameMode, MangoHud (both with 32-bit variants), Steam. The novel pieces beyond that (VRAM release before a Steam/Proton launch, a performance flight recorder, game-triggered theme switching) are planned as opt-in plugins, not shipped yet: see [Roadmap](#roadmap). |
| `dev` | Neovim, tmux, fzf, ripgrep, fd, lazygit. Deeper project-aware tooling (a project-as-profile switch, agent context handoff between projects, environment drift detection) is planned as opt-in plugins, not shipped yet: see [Roadmap](#roadmap). |
