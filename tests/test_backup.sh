...
[[ -f "$NOCTIS_BACKUP_ROOT/20260101-000000/waybar/config.jsonc" ]] || fail "snapshot did not copy waybar config"

for ts in 20260102-000000 20260103-000000 20260104-000000 20260105-000000 20260106-000000; do
  mkdir -p "$NOCTIS_BACKUP_ROOT/$ts"
done

prune_backups 5
remaining=$(ls -1 "$NOCTIS_BACKUP_ROOT" | wc -l)
[[ "$remaining" -eq 5 ]] || fail "expected 5 backups after prune, got $remaining"
[[ ! -d "$NOCTIS_BACKUP_ROOT/20260101-000000" ]] || fail "oldest backup was not pruned"
