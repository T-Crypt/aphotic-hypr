...
NOGPU_FAKEBIN="$(mktemp -d)"
cat > "$NOGPU_FAKEBIN/lspci" <<'FAKE'
#!/usr/bin/env bash
echo "00:02.0 VGA compatible controller: Intel Corporation Raptor Lake-S GT1 [UHD Graphics 770] (rev 04)"
FAKE
chmod +x "$NOGPU_FAKEBIN/lspci"

trap 'rm -rf "$FAKEBIN" "$NOGPU_FAKEBIN"' EXIT
...
output=$(PATH="$NOGPU_FAKEBIN:$PATH" bash install.sh --dry-run --profile minimal --with ai --with-assistant --theme default < /dev/null 2>&1)
