...
// repo (thunar_wall.py, wallswitcher.py): they all copy the chosen image
// to ~/.config/awww/current-wallpaper as the "current wallpaper" marker
// -- not a text file, an actual copy of the image itself.
Singleton {
    id: root

    readonly property string path: `${Quickshell.env("HOME")}/.config/awww/current-wallpaper`
    // Query-string cache-bust: an Image with a `source` URL that never
    // changes won't notice the file's bytes changed underneath it
    // (current-wallpaper is the same path every time, just re-copied).
