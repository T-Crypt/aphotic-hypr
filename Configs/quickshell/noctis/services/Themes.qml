...
// within it) persists to ~/.local/state/noctis/theme.json so SUPER+W-style
...
Singleton {
    ...
    // Each entry: { name, displayName, description, backend, palette,
    ...
    function setTheme(themeName: string, wallpaperFile: string): void {
        ...
        Wallpapers.setWallpaper(fullPath, info.backend ?? "", info.palette ?? "");
        root._saveState();
    }
