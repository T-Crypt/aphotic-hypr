-- Generated by matugen — do not edit by hand, re-run to regenerate.
-- matugen's template syntax only triggers on a double-brace pair, so
-- Lua's own curly-brace table syntax below passes through untouched.
return {
    background = "{{colors.surface.default.hex}}",
    foreground = "{{colors.on_surface.default.hex}}",
    color0  = "{{colors.surface_container_lowest.default.hex}}",
    color1  = "{{colors.error.default.hex}}",
    color2  = "{{colors.tertiary.default.hex}}",
    color3  = "{{colors.secondary.default.hex}}",
    color4  = "{{colors.primary.default.hex}}",
    color5  = "{{colors.primary_fixed_dim.default.hex}}",
    color6  = "{{colors.secondary_fixed_dim.default.hex}}",
    color7  = "{{colors.on_surface_variant.default.hex}}",
    color8  = "{{colors.outline.default.hex}}",
    color9  = "{{colors.on_error_container.default.hex}}",
    color10 = "{{colors.tertiary_fixed.default.hex}}",
    color11 = "{{colors.secondary_fixed.default.hex}}",
    color12 = "{{colors.primary_fixed.default.hex}}",
    color13 = "{{colors.on_primary_container.default.hex}}",
    color14 = "{{colors.on_tertiary_container.default.hex}}",
}
